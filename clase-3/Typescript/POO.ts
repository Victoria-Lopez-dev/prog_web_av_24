...
let listaDelDia:Array<PacienteInfo>=[]
...
class Recepcion{
    ...
    cargarPaciente(paciente:PacienteInfo){
    ...
    //this hace referencia a la clase (en este caso Recepcion)
}
...
class ProfesionalBucal{
    ...
    constructor(correo:string,matricula:number,nombre:string){
        this.correo=correo;
...
class Asistente extends ProfesionalBucal{
        ...
        //atributos 
        diasDisponible=["lun","mier","viernes"];
        horarios:(number)[];
        ...
        constructor(horariosAsist:(number)[],correo:string,matricula:number,nombre:string){
